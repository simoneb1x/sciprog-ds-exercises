...
class BinaryTree:
    ...
    def union_rec(self, other):
        ...
        while len(stack) > 0 or len(stack_other) > 0:
            if len(stack) > 0:
                node = stack.pop()
                node_other = stack_other.pop()
                
                node._data += node_other._data
                ...
                # OTHER
                if node_other.left() != None:
                    stack_other.append(node_other.left())
                if node_other.right() != None:
                    stack_other.append(node_other.right())
            else:
                if len(stack_other) > 0:
          """           
